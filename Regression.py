...
def session(dataset_path, train_path='', test_path=''):
    ...
    with g1.as_default():
        ...
        with tf.name_scope('prediction'):
            ...
            acc = acc_regression(Threshold=1, y_true=y, y_pred=output)
            ...
            for data in input(dataset=train_data, batch_size=500):
                ...
                if i % 100 == 0:
                    loss_ = sess.run(loss, feed_dict={x_f: data[:, :4], x_l: data[:, 4:-1], y: data[:, -1][:, np.newaxis],
                                                      is_training: False})
                    acc_1 = sess.run(acc, feed_dict={x_f: data[:, :4], x_l: data[:, 4:-1], y: data[:, -1][:, np.newaxis],
                                                     is_training: False})
            if i % 100 == 0:
                acc_2 = sess.run(acc, feed_dict={x_f: test_data[:, :4], x_l: test_data[:, 4:-1],
...
if __name__ == '__main__':
    ...
    # SaveFile(data=dataset_guiyi, savepickle_p=r'/home/xiaosong/桌面/PNY_fftguiyi.pickle')

    p = r'/home/xiaosong/桌面/PNY_fftguiyi.pickle'
